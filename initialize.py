...
if file_exists is False: 
 for i,tweet in enumerate(sntwitter.TwitterSearchScraper('100DaysOfCode LearnWeb3DAO').get_items()):
    ...
    if term in input and tweet.date.day == 25:   
...
#use a hash mapping to update the tweets
if file_exists is True:
 for value in df2.values:
  if value in usernameArray:
   value = df['DaysOfCoding'][counter]
   df['DaysOfCoding'][counter] = value + 1
   df['Date Created'][counter] = (2022, 12, 25)
...
#Increment on excel sheet
#Tweet has to be from today
#Has to tag 